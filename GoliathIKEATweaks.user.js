...
//ADDING BACK NAME
if(url == ("https://goliath.hypixel.net/applypunishment"))
{
    var nameString = $('#content')[0].childNodes[7].innerHTML;
    var name = nameString.substring(14,nameString.indexOf("'s "));
    console.log(nameString);
    document.getElementById('field_player').value=name;
}
...
if(url.includes("https://goliath.hypixel.net/profile"))
{
    ...
    $("<h3>SHARE YOUR THEME</h3><p class='currentThemeText'>Currect theme:</p><div class='bodyTextSelector' id='colorSelector'><div class='color-option' style='background:#"+navColor+";cursor:default;'></div><div class='color-option' style='background:#"+textColor+";cursor:default;'></div><div class='color-option' style='background:#"+backgroundColor+";cursor:default;'></div><div class='color-option' style='background:#"+bodyTextColor+";cursor:default;'></div></div><input class='enterTheme' style='margin: 10px 0 10px 0;' type='text' maxlength='27'><div class='themeButtons' id='textColorButtons' style='display:flex;flex-direction:row;'><div class='selectTheme' style='height: 25px;display:flex;flex-direction:colum;justify-content:center;align-items:center;cursor:pointer;margin: 2.5px 0 0 0;border: 1px solid white;font-size:90%;border-radius:4px;width: 110px;'>Select theme</div><div class='copyTheme' style='height: 25px;display:flex;flex-direction:colum;justify-content:center;align-items:center;cursor:pointer;margin: 2.5px 0 0 8px;border: 1px solid white;font-size:90%;border-radius:4px;width: 130px;'>Copy your theme</div></div>").insertAfter("#textColorButtons");     document.getElementsByClassName('sendBackgroundTheme')[0].addEventListener('click', changeBackgroundThemeColor, false);
...
if(url == "https://goliath.hypixel.net/chatreports")
{
    ...
    socket.on("data", function (data) {
        ...
        logs += "<table class=\"uk-table\">";
        logs += "<thead><tr><th>Time</th><th>Server</th><th style='min-width: 20px;'></th><th>Sender</th><th>Message</th></tr></thead>";
        logs += "<tbody>";
        for (var i = 0; i < data.messages.length; i++) {
            var message = data.messages[i];
            if (data.against == message.sender) {
                logs += "<tr style=\"background: #4a4a4a\">"
            } else {
                logs += "<tr>";
            }

            logs += "<td>" + message.timestamp + "</td>";
            logs += "<td>" + message.serverName + "</td>";
            logs += "<td><img src='https://crafatar.com/avatars/" + message.sender + "?size=20'></td>";
            logs += "<td>" + data.formattedUsernames[message.sender] + "</td>";

            var checkingMessage = message.message;
            if(checkingMessage.includes("SHOUT&sect;"))
            {
                checkingMessage = checkingMessage.replace("SHOUT&sect;","<font color='#FFAA00'>[SHOUT]</font> ")
                logs += "<td class=\"message_cell\">" + checkingMessage + "</td>";
            }
            else
            {
                if(checkingMessage.includes("BLOCKED&sect;"))
                {
                    //UGLY FIX
                    checkingMessage = checkingMessage.replace("BLOCKED&sect;","");
                    checkingMessage = "<font color='#910010' text-decoration='none'>[BLOCKED]</font> <font style='text-decoration: line-through;'>"+checkingMessage+"</font>"
                    logs += "<td class=\"message_cell\">" + checkingMessage + "</td>";
                }
                else
                {
        ...
        $("#info").html("<table>\
                        <tr>\
                                <td>Reporter</td>\
                                <td><img src='https://crafatar.com/avatars/" + data.by + "?size=14'> <a target='_blank' href='https://goliath.hypixel.net/userinfo?uuid=" + data.by + "'>" + data.formattedUsernames[data.by] + "</a></strong></td>\
                        </tr>\
                        <tr>\
                                <td>Against</td>\
                                <td><img src='https://crafatar.com/avatars/" + data.against + "?size=14'> <a target='_blank' href='https://goliath.hypixel.net/userinfo?uuid=" + data.against + "'>" + data.formattedUsernames[data.against] + "</a></strong></td>\
                        </tr>\
                        <tr>\
                                <td>Timestamp</td>\
        ...
        // load the uuid, so we don't falseban
        $("#punisher").attr("src", "https://goliath.hypixel.net/punish?player=" + data.against + "&iframe=true");

        var mutesHtml = "<strong>Previous Mutes</strong><br />";

        data.mutes.sort(sortTimestamp);
        data.bans.sort(sortTimestamp);
        ...
        for (var i = 0; i < data.mutes.length; i++) {
            ...
            {
                mutesHtml += "<font color='#abafb5'>"+mute.timestamp + "<br />" + mute.duration + "<br />" + mute.reason + "<br /><br /></font>";
            }
            else
            {
                mutesHtml += mute.timestamp + "<br />" + mute.duration + "<br />" + mute.reason + "<br /><br />";
            }
        }
        if (data.mutes.length === 0) {
            mutesHtml += "No previous mutes.<br />";
        }

        mutesHtml += "<br /><strong>Previous Bans</strong><br />";

        for (var i = 0; i < data.bans.length; i++) {
            var ban = data.bans[i];
            var date1=new Date(ban.timestamp);
            var date2=new Date();
            var year1=date1.getFullYear();
            var year2=date2.getFullYear();
            var month1=date1.getMonth();
        ...
        if (data.bans.length === 0) {
            mutesHtml += "No previous bans.<br />";
        }

        $("#previousMutes").html(mutesHtml);
